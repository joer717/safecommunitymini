		...
		methods:{
			async openInfo(){
				...
				if( KnowledgeHtml.data.status === 200){
					//跳转页面
					uni.navigateTo({
						url: "/pages/webview/index?detailData="+encodeURIComponent(JSON.stringify(KnowledgeHtml.data.body.list[0]))+"&content=" + encodeURIComponent(JSON.stringify(this.KnowledgeHtml))
					})
				}
